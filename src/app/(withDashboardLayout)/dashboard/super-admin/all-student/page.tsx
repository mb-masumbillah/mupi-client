...
import { Search, ChevronLeft, ChevronRight, X, Edit, Trash2, Eye, Download } from "lucide-react";
...
const AllStudent = () => {
  ...
  const [modalType, setModalType] = useState<"view" | "update" | "delete" | null>(null);
  ...
  const approvedStudents = students.filter((s: any) => s.users.status === "approved") || [];

  // ================= Filter & Search =================
  const filteredStudents = approvedStudents.filter((s: any) => {
    const q = search.toLowerCase();
    const matchSearch = s.fullName?.toLowerCase().includes(q) || s.roll?.toString().includes(q);
...
//         await updateStudent(email, new FormData());
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-semibold flex items-center justify-between">
        ...
        <button onClick={handleDownloadExcel} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
      ...
      <div className="flex flex-col md:flex-row md:justify-between items-center gap-4 p-4 border-b border-gray-300 sticky top-0 bg-white z-10">
        <div className="flex flex-wrap gap-2 md:gap-4">
          <select value={deptFilter} onChange={(e) => { setDeptFilter(e.target.value); setPage(1); }} className="px-3 py-2 text-sm border border-slate-400 rounded-md">
            ...
            {[...new Set(approvedStudents.map((s) => s.department))].map((d) => (
          ...
          <select value={sessionFilter} onChange={(e) => { setSessionFilter(e.target.value); setPage(1); }} className="px-3 py-2 text-sm border border-slate-400 rounded-md">
      ...
      <div className="bg-white border border-slate-400 rounded-lg shadow-sm overflow-x-auto">
        <table className="w-full text-sm border-collapse min-w-[600px] md:min-w-full">
          <thead className="bg-slate-200 border-b border-slate-400">
            <tr className="text-slate-800">
              {["Name","Roll","Department","Semester","Shift","Date","Status","Action"].map((h) => (
                <th key={h} className={`px-4 py-3 font-semibold ${h==="Action"?"text-center":"text-left"} border-r last:border-r-0 border-slate-400`}>{h}</th>
              ))}
            </tr>
          </thead>
          ...
          <tbody>
            {paginatedData.map((s, i) => (
              <tr key={i} className="border-b hover:bg-slate-50">
                ...
                <td className="px-4 py-3 border-r text-center">{s.users.status}</td>
                <td className="px-4 py-3">
                  <div className="flex justify-center gap-2">
                    <button title="View Details" onClick={() => { setSelectedStudent(s); setModalType("view"); }} className="p-2 bg-blue-500 text-white rounded hover:bg-blue-600"><Eye size={16} /></button>
                    <button title="Update" onClick={() => openUpdateModal(s)} className="p-2 bg-green-500 text-white rounded hover:bg-green-600"><Edit size={16} /></button>
                    <button title="Delete" onClick={() => { setSelectedStudent(s); setModalType("delete"); }} className="p-2 bg-red-500 text-white rounded hover:bg-red-600"><Trash2 size={16} /></button>
                  </div>
                </td>
              </tr>
            ))}
            {paginatedData.length===0 && (
              <tr><td colSpan={8} className="text-center py-4 text-gray-500">No student found</td></tr>
            )}
          </tbody>
        </table>
        ...
        <div className="flex justify-end items-center gap-2 p-3 border-t">
          ...
          {Array.from({length:totalPages}).map((_,i)=>(<button key={i} onClick={()=>setPage(i+1)} className={`px-3 py-1 border rounded ${page===i+1?"bg-slate-700 text-white border-slate-700":"border-slate-400"}`}>{i+1}</button>))}
      ...
      {modalType==="view" && selectedStudent && (
        <div className="fixed inset-0 bg-black/30 flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg max-h-[34rem] overflow-y-auto p-6 relative w-full max-w-3xl">
            <button className="absolute top-3 right-3 text-gray-500 hover:text-gray-700" onClick={()=>setModalType(null)}><X /></button>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <p><strong>Phone:</strong> {selectedStudent.number}</p>
              <div className="col-span-2 flex justify-center mt-4">
                {selectedStudent.image && <Image src={selectedStudent.image} alt="Student" width={150} height={150} className="rounded-xl" />}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/30 flex justify-center items-center z-50 p-4 overflow-auto">
          <div className="bg-white rounded-lg shadow-lg max-h-[90vh] overflow-y-auto p-6 relative w-full max-w-3xl">
            ...
            <form onSubmit={handleSubmit(onUpdateSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <TextInput label="Roll" name="roll" type="number" control={control} />
              <TextInput label="Registration" name="registration" type="number" control={control} />
              <SelectInput label="Department" name="department" control={control} options={["Computer","Electrical","Civil","Mechanical","IPCT","Electronics","Electro_Medical","RAC"]} />
              <TextInput label="Session" name="session" control={control} />
              <SelectInput label="Shift" name="shift" control={control} options={["First","Second"]} />
              <SelectInput label="Semester" name="semester" control={control} options={["First","Second","Third","Fourth","Fifth","Sixth","Seventh","Eighth"]} />
              <TextInput label="Email" name="email" control={control} />
              <TextInput label="Phone" name="number" control={control} />
              <ImageUpload label="Student Image" name="image" control={control} />
              <div className="md:col-span-2 flex justify-center mt-4">
                <button type="submit" className="px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700">Update</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {modalType==="delete" && selectedStudent && (
        <div className="fixed inset-0 bg-black/30 flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg p-6 relative w-full max-w-md">
            ...
            <p>Choose the type of deletion for <strong>{selectedStudent.fullName}</strong>:</p>
            <div className="flex justify-center gap-4 mt-6">
              <button onClick={()=>handleSoftDelete(selectedStudent.email)} className="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600">Soft Delete</button>
              <button onClick={()=>handlePermanentDelete(selectedStudent.email)} className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Permanent Delete</button>
            </div>
          </div>
        </div>
      )}
